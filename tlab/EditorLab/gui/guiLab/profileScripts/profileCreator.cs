...
function GLab::createProfileClone(%this ) {    
   ...
   if( %name $= "" ){
      msgBoxOk("Invalid profile name","The name of the new profile is invalid. Please make sure to enter a valid name. Name was:" SPC %name);
      return;
   }
   if( !isObject(%cloneSrc)){
      msgBoxOk("Invalid profile source","The specified profile source:" SPC %cloneSrc SPC "is not a valid profile, please choose a valid one");
      return;
   }
   
   if( isObject( %name ) ){
      msgBoxOk("Name already use","The name you have choose for your profile is already used by another game object. Please choose another.");
      return;
   }
