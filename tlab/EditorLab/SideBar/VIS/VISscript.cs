...
function SideBarVIS::toggleVisibility(%this) {
	devLog("SideBarVISvisibility is",%dlg.visible);
...
//------------------------------------------------------------------------------
function SideBarVIS::updateOptions( %this ) {
...
function SideBarVIS::initOptions( %this ) {
	ar_SideBarVIS.empty();
...
function SideBarVIS::addClassOptions( %this ) {
	...
	for ( %i = 0; %i < %this.classArray.count(); %i++ ) {
		...
		%visCheckBox = new GuiCheckBoxCtrl() {
			...
			Visible = "1";
			Variable = %visVar;
			command = "SideBarVIS.toggleRenderable(\""@%class@"\");";
			tooltipprofile = "ToolsToolTipProfile";
			hovertime = "1000";
		...
		%selCheckBox = new GuiCheckBoxCtrl() {
			...
			canSave = "1";
			Visible = "1";
			command = "SideBarVIS.toggleSelectable(\""@%class@"\");";
			Variable = %selVar;
			tooltipprofile = "ToolsToolTipProfile";
			hovertime = "1000";
